...
//
// $Id: num_util.h,v 1.18 2007/07/02 18:17:15 pfkeb Exp $
//
...
namespace num_util{
...
#ifdef HAVE_NUMPY
#ifdef _MSC_VER
  template <typename T>  PyArray_TYPES getEnum ();
#else
  ...
  memcpy(arr_data, data, sizeof(T) * n); // copies the input data to 
                                           // PyArrayObject->data
#endif
...
#else
  char *arr_data = ((PyArrayObject*) obj.ptr())->data; 
  memcpy(arr_data, data, sizeof(T) * total); // copies the input data to 
                                           // PyArrayObject->data
#endif
  ...
  template <>  boost::python::numeric::array makeNum<double> ( double * data,
						  std::vector < int>  dims );    
